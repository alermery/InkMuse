...
- 数据存储：创作数据以浏览器本地存储为主；API Key 默认只保存在当前页面会话中，用户主动开启“记住此设备”后才会写入 LocalStorage
...
这种方式下，Key 默认只保存在当前页面会话中，用于调用站点的代理接口。关闭或刷新页面后，未开启持久化的 Key 需要重新输入。
...
- 只有用户开启“记住此设备”后，API Key 才会写入当前浏览器的 LocalStorage，并经过简单混淆后存储。
- 浏览器端存储不等同于高强度加密，不建议在公共设备上长期保存 Key。
- 如果用户关闭“记住此设备”或点击“清除本机 Key”，本地持久化的 Key 会被移除。
...
当前版本没有账号同步能力。创作内容、收藏素材和设定集主要保存在当前浏览器环境中。API Key 默认不参与长期保存，只有主动开启“记住此设备”后才会留在当前浏览器。
